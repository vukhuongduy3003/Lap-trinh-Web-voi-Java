...
public class HoaDonServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            List<HoaDon> hoaDons = getHoaDons();

            request.setAttribute("hoadons", hoaDons);

            request.setAttribute("test", "test");
            request.getRequestDispatcher("/hoadons.jsp").forward(request, response);
    ...
    private List<HoaDon> getHoaDons() throws SQLException, ClassNotFoundException {
        List<HoaDon> hoaDons = new ArrayList<HoaDon>();
        Connector c =  new Connector();
        ResultSet rs = null;
        rs = c.getData("tt_hoadon");
        while(rs.next())
         {
             HoaDon s =  new HoaDon();
             s.setMahdon(rs.getString("mahdon"));
             s.setNgaylap(rs.getDate("ngaylap"));
             s.setSokw(rs.getInt("sokw"));
